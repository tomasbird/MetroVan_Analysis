...
metrics %>% 
  select(!c('Direction', 'Distance')) %>%
  pivot_longer(!c( 'Rep', 'Station', 'Year'), names_to="Indicator", values_to="value") %>%
  ggplot(aes(x=value)) +
    geom_histogram()+
...
Here we should be plotting response variables against predictors.  
...
metrics %>% 
  select(!c('Direction')) %>%
  pivot_longer(!c( 'Rep', 'Station', 'Year', 'Distance'), names_to="Indicator", values_to="value") %>%
